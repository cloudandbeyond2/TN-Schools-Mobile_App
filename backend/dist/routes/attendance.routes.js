...
router.post('/', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const result = yield prisma_1.prisma.$transaction([
            prisma_1.prisma.attendance.deleteMany({
                where: {
            ...
            prisma_1.prisma.attendance.createMany({
                data: records.map(r => ({
        ...
        if (notifySMS) {
            ...
            for (const record of absentRecords) {
                try {
                    ...
                    const parents = yield (0, sms_1.getStudentParents)(studentId);
                    if (parents.length > 0) {
                        const student = yield prisma_1.prisma.student.findUnique({
                            where: { id: studentId },
                            include: { user: true, school: true },
                        });
                        const studentName = ((_a = student === null || student === void 0 ? void 0 : student.user) === null || _a === void 0 ? void 0 : _a.name) || 'Your child';
                        const schoolName = ((_b = student === null || student === void 0 ? void 0 : student.school) === null || _b === void 0 ? void 0 : _b.name) || 'School';
        ...
        try {
            ...
            for (const studentId of studentIds) {
                const monthlyRecords = yield prisma_1.prisma.attendance.findMany({
                    where: {
                        studentId,
                ...
                if (monthlyRecords.length >= 3) {
                    ...
                    if (rate < 75) {
                        ...
                        for (const parent of parents) {
                            if (parent.id) {
                                const existingAlert = yield prisma_1.prisma.parentNotification.findFirst({
                                    where: {
                                        parentId: parent.id,
                                        studentId,
                                ...
                                if (!existingAlert) {
                                    const alertMsg = `Attendance Warning: Your child ${((_c = student === null || student === void 0 ? void 0 : student.user) === null || _c === void 0 ? void 0 : _c.name) || 'child'} has low attendance (${Math.round(rate)}%) for this month. Please ensure they attend school regularly.`;
                                    yield (0, sms_1.sendMockSMS)(parent.phone, alertMsg);
                                    yield prisma_1.prisma.parentNotification.create({
                                        data: {
                                            parentId: parent.id,
                                            studentId,
...
router.get('/class-date', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
    ...
    try {
        ...
        res.json({
            ...
            students: students.map(s => {
                ...
                return {
                    id: s.id,
                    name: ((_a = s.user) === null || _a === void 0 ? void 0 : _a.name) || 'Unknown',
                    rollNo: s.rollNumber || '',
    ...
    try {
        ...
        const attendance = yield prisma_1.prisma.attendance.findMany({
            where: {
                ...
                date: Object.assign(Object.assign({}, (from ? { gte: new Date(String(from)) } : {})), (to ? { lte: new Date(String(to)) } : {})),
            },
            orderBy: { date: 'desc' },
        });
        // Compute %
...
router.get('/school/:schoolId/stats', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    var _a, _b;
    try {
        ...
        const targetRecords = yield prisma_1.prisma.attendance.findMany({
            where: {
                schoolId,
                date: { gte: targetDate, lt: nextDate }
            },
        ...
        for (const record of targetRecords) {
            const clsName = ((_a = record.student) === null || _a === void 0 ? void 0 : _a.class) ? `Class ${record.student.class}${record.student.section || ''}` : 'Unassigned';
            if (!byClass[clsName]) {
                byClass[clsName] = { total: 0, present: 0, absent: 0, late: 0, leave: 0, marked: 0 };
            }
            byClass[clsName].marked += 1;
            if (record.status === 'PRESENT')
                byClass[clsName].present += 1;
            else if (record.status === 'ABSENT')
                byClass[clsName].absent += 1;
            else if (record.status === 'LATE')
                byClass[clsName].late += 1;
            else if (record.status === 'LEAVE')
                byClass[clsName].leave += 1;
        }
        const classWise = Object.entries(byClass).map(([className, counts]) => {
            const pct = counts.total > 0
                ...
                : 0;
            return Object.assign(Object.assign({ className }, counts), { percentage: pct });
        });
        // Late logs
        const lateLogs = targetRecords
            ...
            return ({
                id: r.id,
                studentId: r.studentId,
                name: ((_b = (_a = r.student) === null || _a === void 0 ? void 0 : _a.user) === null || _b === void 0 ? void 0 : _b.name) || 'Unknown',
                rollNumber: ((_c = r.student) === null || _c === void 0 ? void 0 : _c.rollNumber) || 'N/A',
                class: ((_d = r.student) === null || _d === void 0 ? void 0 : _d.class) || 'N/A',
                section: ((_e = r.student) === null || _e === void 0 ? void 0 : _e.section) || 'N/A',
                status: r.status,
        ...
        for (const s of students) {
            ...
            if (pct < 75 || consecutiveAbsent >= 3) {
                alerts.push({
                    studentId: s.id,
                    name: ((_b = s.user) === null || _b === void 0 ? void 0 : _b.name) || 'Unknown',
        ...
        for (let i = 5; i >= 0; i--) {
            ...
            const totalM = yield prisma_1.prisma.attendance.count({
                where: {
                    schoolId,
                    date: { gte: firstDay, lte: lastDay }
                }
            });
            const presM = yield prisma_1.prisma.attendance.count({
                where: {
                    schoolId,
                    date: { gte: firstDay, lte: lastDay },
        ...
        const dailyRecords = yield prisma_1.prisma.attendance.findMany({
            where: {
                schoolId,
                date: { gte: startOfMonth, lte: endOfMonth }
            },
...
router.get('/school/:schoolId/today', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        const tomorrow = new Date(today);
        tomorrow.setDate(tomorrow.getDate() + 1);
        const records = yield prisma_1.prisma.attendance.groupBy({
            by: ['status'],
