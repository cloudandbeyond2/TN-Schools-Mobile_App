...
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield prisma.digitalLibraryResource.deleteMany({});
        yield prisma.personalGuide.deleteMany({});
        yield prisma.nEETChapter.deleteMany({});
        yield prisma.nEETMockTest.deleteMany({});
        yield prisma.competitiveExam.deleteMany({});
        ...
        const teacher = yield prisma.user.findFirst({ where: { role: 'TEACHER' } });
        const teacherId = teacher ? teacher.id : null;
        // 1. Digital Library Resources
        const existingResources = yield prisma.digitalLibraryResource.count();
        if (existingResources === 0) {
            yield prisma.digitalLibraryResource.createMany({
                data: [
                    {
        ...
        const existingGuides = yield prisma.personalGuide.count();
        if (existingGuides === 0) {
            const dbStudents = yield prisma.student.findMany({
                include: { user: true },
                take: 6
            });
            const goals = [
            ...
            if (dbStudents.length > 0) {
                ...
                yield prisma.personalGuide.createMany({
                    data: guideData
                });
        ...
        yield prisma.nEETChapter.createMany({
            data: [
                {
                    subject: "Biology",
        ...
        // 4. NEET Mock Tests
        yield prisma.nEETMockTest.createMany({
            data: [
                {
        ...
        // 5. Competitive Exams
        yield prisma.competitiveExam.createMany({
            data: [
                {
