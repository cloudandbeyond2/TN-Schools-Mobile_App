...
router.param('parentId', (req, res, next, parentId) => __awaiter(void 0, void 0, void 0, function* () {
    try {
...
// ─────────────────────────────────────────────────────────────────
router.get('/:parentId/children', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
...
// ─────────────────────────────────────────────────────────────────
router.get('/:parentId/child/:studentId/summary', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const attendanceRecords = yield prisma_1.prisma.attendance.findMany({ where: { studentId } });
        ...
        if (modelExamResults.length > 0) {
            ...
            const marks = yield prisma_1.prisma.mark.findMany({ where: { studentId } });
        ...
        const allHomework = yield prisma_1.prisma.homework.findMany({
...
// ─────────────────────────────────────────────────────────────────
router.get('/:parentId/child/:studentId/performance', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { studentId } = req.params;
        ...
        if (modelExamResults.length > 0) {
            ...
            for (const r of modelExamResults) {
                ...
                if (r.extraSubject !== null && r.extraSubjectName) {
                    ...
                    if (!bySubject[extraName])
                        bySubject[extraName] = {};
                    bySubject[extraName][examName] = r.extraSubject;
                }
            }
            subjectData = Object.entries(bySubject).map(([subject, examScores]) => (Object.assign({ subject }, examScores)));
            years = [...new Set(modelExamResults.map(r => r.exam.academicYear))].sort();
            rawMarks = modelExamResults.flatMap((r) => {
                const list = [];
                if (r.tamil !== null)
                    list.push({ id: `${r.id}-tamil`, subject: 'Tamil', examType: r.exam.examName, scored: r.tamil, maxMarks: 100, academicYear: r.exam.academicYear });
                if (r.english !== null)
                    list.push({ id: `${r.id}-english`, subject: 'English', examType: r.exam.examName, scored: r.english, maxMarks: 100, academicYear: r.exam.academicYear });
                if (r.mathematics !== null)
                    list.push({ id: `${r.id}-math`, subject: 'Mathematics', examType: r.exam.examName, scored: r.mathematics, maxMarks: 100, academicYear: r.exam.academicYear });
                if (r.science !== null)
                    list.push({ id: `${r.id}-science`, subject: 'Science', examType: r.exam.examName, scored: r.science, maxMarks: 100, academicYear: r.exam.academicYear });
                if (r.socialScience !== null)
                    list.push({ id: `${r.id}-social`, subject: 'Social Science', examType: r.exam.examName, scored: r.socialScience, maxMarks: 100, academicYear: r.exam.academicYear });
                if (r.extraSubject !== null && r.extraSubjectName) {
...
// ─────────────────────────────────────────────────────────────────
router.get('/:parentId/child/:studentId/attendance', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { studentId } = req.params;
        const { offset } = req.query;
        const targetOffset = offset !== undefined ? Number(offset) : 0;
        const monthCount = 6;
        const monthlyData = yield Promise.all(Array.from({ length: monthCount }, (_, i) => i).reverse().map((offsetVal) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        const recentRecords = yield prisma_1.prisma.attendance.findMany({
...
// ─────────────────────────────────────────────────────────────────
router.get('/:parentId/child/:studentId/homework', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { studentId } = req.params;
        ...
        const data = homeworkList.map(h => {
            ...
            return {
                ...
                score: (_a = submission === null || submission === void 0 ? void 0 : submission.score) !== null && _a !== void 0 ? _a : '—',
                feedback: (_b = submission === null || submission === void 0 ? void 0 : submission.feedback) !== null && _b !== void 0 ? _b : null,
                submittedDate: (_c = submission === null || submission === void 0 ? void 0 : submission.date) !== null && _c !== void 0 ? _c : '—',
            };
        });
...
// ─────────────────────────────────────────────────────────────────
router.get('/:parentId/child/:studentId/scholarship', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { studentId } = req.params;
...
// ─────────────────────────────────────────────────────────────────
router.get('/:parentId/notifications', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { parentId } = req.params;
        const { unreadOnly } = req.query;
        const notifications = yield prisma_1.prisma.parentNotification.findMany({
            where: Object.assign({ parentId }, (unreadOnly === 'true' ? { isRead: false } : {})),
            orderBy: { createdAt: 'desc' },
        });
        const unreadCount = yield prisma_1.prisma.parentNotification.count({ where: { parentId, isRead: false } });
...
// ─────────────────────────────────────────────────────────────────
router.put('/:parentId/notifications/:id/read', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { id } = req.params;
        yield prisma_1.prisma.parentNotification.update({ where: { id }, data: { isRead: true } });
...
// ─────────────────────────────────────────────────────────────────
router.put('/:parentId/notifications/read-all', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { parentId } = req.params;
        yield prisma_1.prisma.parentNotification.updateMany({ where: { parentId, isRead: false }, data: { isRead: true } });
        res.json({ success: true });
    }
    catch (err) {
        res.status(500).json({ success: false, error: String(err) });
...
// ─────────────────────────────────────────────────────────────────
router.put('/:parentId/notifications/:id/unread', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { id } = req.params;
        yield prisma_1.prisma.parentNotification.update({ where: { id }, data: { isRead: false } });
        res.json({ success: true });
    }
    catch (err) {
        res.status(500).json({ success: false, error: String(err) });
...
// ─────────────────────────────────────────────────────────────────
router.put('/:parentId/notifications/unread-all', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { parentId } = req.params;
        const { ids } = req.body;
        if (Array.isArray(ids) && ids.length > 0) {
            yield prisma_1.prisma.parentNotification.updateMany({
...
// ─────────────────────────────────────────────────────────────────
router.delete('/:parentId/notifications/:id', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { id } = req.params;
        yield prisma_1.prisma.parentNotification.delete({ where: { id } });
        res.json({ success: true });
    }
    catch (err) {
        res.status(500).json({ success: false, error: String(err) });
...
router.get('/pta-meetings', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const enrichedMeetings = meetings.map(m => (Object.assign(Object.assign({}, m), { rsvps: rsvps[m.id] || {} })));
...
// ─────────────────────────────────────────────────────────────────
router.post('/link', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
...
// ─────────────────────────────────────────────────────────────────
router.delete('/link', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
    ...
    try {
        ...
        if (links.length === 0) {
            ...
            if (student && student.parentMobile) {
                ...
                if (parent) {
                    const notif = yield prisma_1.prisma.parentNotification.create({
                        data: {
                            parentId: parent.id,
                            studentId,
...
// ─────────────────────────────────────────────────────────────────
router.get('/:parentId/child/:studentId/performance-summary', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { studentId } = req.params;
        ...
        const personalGuides = yield prisma_1.prisma.personalGuide.findMany({
            where: { studentId },
            orderBy: { createdAt: 'desc' },
        });
        // 6. Fetch homework submissions with feedback
        const homeworkSubmissions = yield prisma_1.prisma.homeworkSubmission.findMany({
            where: { studentId },
...
Ensure the tone is warm, motivating, and culturally appropriate. Do not use markdown format inside the JSON fields.`;
                ...
                const result = yield (0, ai_routes_1.callGemini)(prompt, true, SCHEMA);
...
function generateRulesFallback(name, avg, attendance, strengths, weaknesses) {
...
router.get('/teachers', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const schoolTeachers = yield prisma_1.prisma.teacher.findMany({
        ...
        const mappedTeachers = schoolTeachers.map(t => {
            ...
            return ({
                id: t.id,
                user: {
                    name: ((_a = t.user) === null || _a === void 0 ? void 0 : _a.name) || 'Unknown Teacher',
                    email: ((_b = t.user) === null || _b === void 0 ? void 0 : _b.email) || null,
...
router.get('/teacher-slots', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { teacherId } = req.query;
        if (!teacherId) {
            return res.status(400).json({ success: false, error: 'teacherId is required' });
        }
        const slots = yield prisma_1.prisma.teacherMeetingSlot.findMany({
...
router.get('/pta-appointments', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { parentId, teacherUserId } = req.query;
        let whereClause = {};
        if (parentId) {
            const resolvedParentId = yield resolveParentId(String(parentId));
        ...
        const appointments = yield prisma_1.prisma.pTAAppointment.findMany({
            where: whereClause,
            orderBy: { createdAt: 'desc' }
        });
        const enriched = yield Promise.all(appointments.map((appt) => __awaiter(void 0, void 0, void 0, function* () {
            var _a, _b;
            ...
            if ((_a = teacher === null || teacher === void 0 ? void 0 : teacher.user) === null || _a === void 0 ? void 0 : _a.name) {
                ...
                if (staff === null || staff === void 0 ? void 0 : staff.name) {
            ...
            return Object.assign(Object.assign({}, appt), { teacherName, parentName: (parent === null || parent === void 0 ? void 0 : parent.name) || 'Unknown Parent', studentName: ((_b = student === null || student === void 0 ? void 0 : student.user) === null || _b === void 0 ? void 0 : _b.name) || 'Unknown Student' });
        })));
...
router.post('/pta-appointments', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    try {
        ...
        const resolvedParentId = yield resolveParentId(String(parentId));
        const appointment = yield prisma_1.prisma.pTAAppointment.create({
            data: {
        ...
        if (teacher) {
            if ((_a = teacher.user) === null || _a === void 0 ? void 0 : _a.name) {
                teacherName = teacher.user.name;
            }
...
router.put('/pta-appointments/:id/status', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    try {
        const { id } = req.params;
        ...
        const appt = yield prisma_1.prisma.pTAAppointment.update({
            where: { id },
            data: { status, notes }
        });
...
// ─────────────────────────────────────────────────────────────────
router.get('/scholarship-schemes', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const schemes = yield prisma_1.prisma.scholarshipScheme.findMany({
            orderBy: { name: 'asc' },
        });
