...
const STAFF_ROLES = ['TEACHER', 'HEADMASTER', 'BEO', 'DEO', 'COMMISSIONER', 'MINISTER', 'SUPERADMIN'];
const OVERSIGHT_ROLES = ['HEADMASTER', 'BEO', 'DEO', 'COMMISSIONER', 'MINISTER', 'SUPERADMIN'];
const SSLC_SUBJECTS = ['Tamil', 'English', 'Mathematics', 'Science', 'Social Science'];
const isStaffRole = (req) => {
...
router.get('/papers', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { class: cls, subject, schoolId } = req.query;
        const filter = {};
        if (cls)
            filter.class = cls;
        ...
        const papers = yield mongo_1.SSLCQuestionPaper.find(filter).sort({ year: -1, createdAt: -1 });
...
router.post('/papers', (0, auth_middleware_1.requireRole)(STAFF_ROLES), (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { class: cls, subject, year, paperType, title, fileUrl, durationMinutes, maxMarks, schoolId, uploadedById, uploadedByName } = req.body;
        if (!cls || !subject || !year || !title) {
            return res.status(400).json({ success: false, error: 'class, subject, year and title are required' });
        }
        const paper = yield mongo_1.SSLCQuestionPaper.create({
...
router.delete('/papers/:id', (0, auth_middleware_1.requireRole)(STAFF_ROLES), (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        yield mongo_1.SSLCQuestionPaper.findByIdAndDelete(req.params.id);
...
router.post('/papers/:id/download', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const paper = yield mongo_1.SSLCQuestionPaper.findByIdAndUpdate(req.params.id, { $inc: { downloads: 1 } }, { new: true });
        res.json({ success: true, data: paper });
    }
    catch (err) {
        res.status(500).json({ success: false, error: String(err) });
...
router.get('/plans', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { class: cls, subject, schoolId, includeDrafts } = req.query;
        const filter = {};
        if (cls)
            filter.class = cls;
        if (subject && subject !== 'All')
        ...
        if (!(includeDrafts === 'true' && isStaffRole(req)))
            filter.published = true;
        let plans = yield mongo_1.SSLCPrepPlan.find(filter).sort({ subject: 1, createdAt: -1 });
        // Auto-seed high-yield state board prep plans if collection is empty
        if (plans.length === 0) {
            yield seedMasterPrepPlans();
            plans = yield mongo_1.SSLCPrepPlan.find(filter).sort({ subject: 1, createdAt: -1 });
        }
...
router.post('/plans', (0, auth_middleware_1.requireRole)(STAFF_ROLES), (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const plan = yield mongo_1.SSLCPrepPlan.create({
...
router.put('/plans/:id', (0, auth_middleware_1.requireRole)(STAFF_ROLES), (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (cls !== undefined)
            update.class = String(cls);
        const plan = yield mongo_1.SSLCPrepPlan.findByIdAndUpdate(req.params.id, update, { new: true });
        res.json({ success: true, data: plan });
    }
    catch (err) {
        res.status(500).json({ success: false, error: String(err) });
...
router.patch('/plans/:id/publish', (0, auth_middleware_1.requireRole)(STAFF_ROLES), (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const plan = yield mongo_1.SSLCPrepPlan.findByIdAndUpdate(req.params.id, { published: !!req.body.published }, { new: true });
        res.json({ success: true, data: plan });
    }
    catch (err) {
        res.status(500).json({ success: false, error: String(err) });
...
router.delete('/plans/:id', (0, auth_middleware_1.requireRole)(STAFF_ROLES), (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        yield mongo_1.SSLCPrepPlan.findByIdAndDelete(req.params.id);
        res.json({ success: true });
    }
    catch (err) {
        res.status(500).json({ success: false, error: String(err) });
...
const sanitizeTestForStudent = (test) => {
    const obj = test.toObject ? test.toObject() : Object.assign({}, test);
    obj.questions = (obj.questions || []).map((q) => {
        const { answer } = q, rest = __rest(q, ["answer"]);
        return rest;
    });
    return obj;
};
...
router.get('/mock-tests', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { class: cls, subject, schoolId, all } = req.query;
        const filter = {};
        if (cls)
            filter.class = cls;
        if (subject && subject !== 'All')
        ...
        if (!staff)
            filter.published = true;
        const tests = yield mongo_1.SSLCMockTest.find(filter).sort({ createdAt: -1 });
        const data = staff
            ? tests
            : tests.map((t) => {
...
router.get('/mock-tests/:id', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const test = yield mongo_1.SSLCMockTest.findById(req.params.id);
        if (!test)
...
router.post('/mock-tests', (0, auth_middleware_1.requireRole)(STAFF_ROLES), (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { schoolId, class: cls, subject, title, durationMinutes, totalMarks, difficulty, questions, published, createdById, createdByName } = req.body;
        ...
        const test = yield mongo_1.SSLCMockTest.create({
            schoolId: schoolId || undefined,
...
router.patch('/mock-tests/:id/publish', (0, auth_middleware_1.requireRole)(STAFF_ROLES), (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const test = yield mongo_1.SSLCMockTest.findByIdAndUpdate(req.params.id, { published: !!req.body.published }, { new: true });
        res.json({ success: true, data: test });
    }
    catch (err) {
        res.status(500).json({ success: false, error: String(err) });
...
router.delete('/mock-tests/:id', (0, auth_middleware_1.requireRole)(STAFF_ROLES), (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        yield mongo_1.SSLCMockTest.findByIdAndDelete(req.params.id);
        yield mongo_1.SSLCMockAttempt.deleteMany({ testId: req.params.id });
        res.json({ success: true });
    }
    catch (err) {
        res.status(500).json({ success: false, error: String(err) });
...
router.post('/mock-tests/:id/attempts', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    var _a, _b, _c;
    try {
        ...
        const selfMap = selfMarks || {};
        for (const q of test.questions) {
            maxScore += q.marks;
            if (q.type === 'mcq') {
                const given = String((_a = answerMap[q.qid]) !== null && _a !== void 0 ? _a : '').trim().toUpperCase();
                const expected = String((_b = q.answer) !== null && _b !== void 0 ? _b : '').trim().toUpperCase();
                ...
                score += awarded;
                if (awarded >= q.marks * 0.5)
                    correctCount += 1;
            }
        }
        const percentage = maxScore > 0 ? Math.round((score / maxScore) * 1000) / 10 : 0;
        const attempt = yield mongo_1.SSLCMockAttempt.create({
            testId: String(test._id),
            studentId: (student === null || student === void 0 ? void 0 : student.id) || studentId,
            studentName: (_c = student === null || student === void 0 ? void 0 : student.user) === null || _c === void 0 ? void 0 : _c.name,
            schoolId: (student === null || student === void 0 ? void 0 : student.schoolId) || test.schoolId,
            class: test.class,
...
router.get('/mock-tests/:id/attempts', (0, auth_middleware_1.requireRole)(STAFF_ROLES), (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const attempts = yield mongo_1.SSLCMockAttempt.find({ testId: req.params.id }).sort({ createdAt: -1 });
...
router.get('/attempts', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const attempts = yield mongo_1.SSLCMockAttempt.find({
            studentId: { $in: [studentId, student === null || student === void 0 ? void 0 : student.id].filter(Boolean) },
        }).sort({ createdAt: -1 });
        res.json({ success: true, data: attempts });
    }
    catch (err) {
...
function collectSubjectHistory(studentDbId) {
    return __awaiter(this, void 0, void 0, function* () {
        var _a;
        const history = {};
        const push = (subject, percent, at, source) => {
        ...
        const marks = yield prisma_1.prisma.mark.findMany({ where: { studentId: studentDbId } });
        for (const m of marks) {
            if (m.maxMarks > 0)
                push(m.subject, (m.scored / m.maxMarks) * 100, m.createdAt, m.examType);
        }
        const modelResults = yield prisma_1.prisma.modelExamResult.findMany({
        ...
        for (const r of modelResults) {
            ...
            for (const [subject, scored] of subjectCols) {
                if (scored !== null && scored !== undefined) {
                    push(subject, scored, r.createdAt, ((_a = r.exam) === null || _a === void 0 ? void 0 : _a.examName) || 'Model Exam');
                }
            }
        }
        const attempts = yield mongo_1.SSLCMockAttempt.find({ studentId: studentDbId });
        for (const a of attempts) {
            push(a.subject, a.percentage, a.createdAt, `Mock: ${a.testTitle}`);
        }
        for (const key of Object.keys(history))
            history[key].sort((a, b) => a.at - b.at);
        return history;
    });
}
// Least-squares linear regression over (index, percent) points,
// projected one step ahead. Returns clamped [0, 100].
function predictNextPercent(points) {
    if (points.length === 0)
        return { predicted: 0, trend: 0 };
    if (points.length === 1)
        return { predicted: points[0].percent, trend: 0 };
...
function buildPrediction(studentDbId) {
    return __awaiter(this, void 0, void 0, function* () {
        const history = yield collectSubjectHistory(studentDbId);
...
function buildPredictionsBatch(students, allMarks, allModelResults, allAttempts) {
    var _a;
    ...
    const modelResultsByStudent = {};
    for (const r of allModelResults) {
        if (!modelResultsByStudent[r.studentId])
            modelResultsByStudent[r.studentId] = [];
        modelResultsByStudent[r.studentId].push(r);
    }
    const attemptsByStudent = {};
    for (const a of allAttempts) {
        if (!attemptsByStudent[a.studentId])
            attemptsByStudent[a.studentId] = [];
...
/* ════════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════════ */
router.get('/predictions/:studentId', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    try {
        ...
        const prediction = yield buildPrediction(student.id);
        res.json({
            success: true,
            data: Object.assign({ studentId: student.id, studentName: (_a = student.user) === null || _a === void 0 ? void 0 : _a.name, class: student.class }, prediction),
        });
    }
    catch (err) {
        res.status(500).json({ success: false, error: String(err) });
...
router.get('/revision-plan/:studentId', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { studentId } = req.params;
        ...
        if (!student)
            return res.status(404).json({ success: false, error: 'Student not found' });
        const plan = yield mongo_1.SSLCRevisionPlan.findOne({ studentId: student.id, class: cls });
        res.json({ success: true, data: plan });
    }
    catch (err) {
        res.status(500).json({ success: false, error: String(err) });
...
router.post('/revision-plan/generate', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (!student)
            return res.status(404).json({ success: false, error: 'Student not found' });
        const prediction = yield buildPrediction(student.id);
        const prepDoc = yield mongo_1.BoardPrep.findOne({ studentId: student.id, class: selectedClass });
        // Weakness score per subject: low predicted % and low syllabus
        // completion both raise priority.
        const ranked = prediction.subjects
            .map((s) => {
            var _a;
            const syllabusItem = (_a = prepDoc === null || prepDoc === void 0 ? void 0 : prepDoc.syllabusProgress) === null || _a === void 0 ? void 0 : _a.find((p) => p.subject.toLowerCase() === s.subject.toLowerCase());
            ...
            return Object.assign(Object.assign({}, s), { completionPct, weakness: (100 - performance) * 0.7 + (100 - completionPct) * 0.3 });
        })
        ...
        const plan = yield mongo_1.SSLCRevisionPlan.findOneAndUpdate({ studentId: student.id, class: selectedClass }, {
...
router.patch('/revision-plan/:studentId/task', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    try {
        const { studentId } = req.params;
        ...
        if (!student)
            return res.status(404).json({ success: false, error: 'Student not found' });
        const plan = yield mongo_1.SSLCRevisionPlan.findOne({ studentId: student.id, class: String(cls || '10') });
        if (!plan)
            return res.status(404).json({ success: false, error: 'Revision plan not found' });
        const day = plan.days[Number(dayIndex)];
        const task = (_a = day === null || day === void 0 ? void 0 : day.tasks) === null || _a === void 0 ? void 0 : _a[Number(taskIndex)];
        if (!task)
            return res.status(400).json({ success: false, error: 'Invalid day/task index' });
        task.done = !!done;
        yield plan.save();
        res.json({ success: true, data: plan });
    }
...
/* ════════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════════ */
router.get('/analytics/school', (0, auth_middleware_1.requireRole)(STAFF_ROLES), (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { schoolId, class: cls, section, teacherId } = req.query;
        if (!schoolId)
        ...
        if (teacherId) {
            ...
            const teacherClassrooms = yield prisma_1.prisma.classRoom.findMany({
                where: {
                    schoolId,
            ...
            teacherConditions = Object.assign({ class: { in: classes } }, (section ? { section } : {}));
        }
        const students = yield prisma_1.prisma.student.findMany({
            where: Object.assign({ schoolId }, teacherConditions),
            include: { user: true },
        });
        const studentIds = students.map((s) => s.id);
        // Syllabus readiness from BoardPrep docs.
        const prepDocs = yield mongo_1.BoardPrep.find({ studentId: { $in: studentIds } });
        let readinessSum = 0;
        ...
        const attempts = yield mongo_1.SSLCMockAttempt.find({ studentId: { $in: studentIds } });
        ...
        const [allMarks, allModelResults] = yield Promise.all([
            prisma_1.prisma.mark.findMany({ where: { studentId: { in: activeStudentIds } } }),
            prisma_1.prisma.modelExamResult.findMany({
                where: { studentId: { in: activeStudentIds } },
                include: { exam: true }
            })
        ]);
        const subsetAttempts = attempts.filter(a => activeStudentIds.includes(a.studentId));
        const batchPredictions = buildPredictionsBatch(activeStudentSubset, allMarks, allModelResults, subsetAttempts);
        const perStudent = activeStudentSubset.map((s) => {
            var _a, _b;
            const prediction = batchPredictions[s.id];
            return {
                studentId: s.id,
                name: ((_a = s.user) === null || _a === void 0 ? void 0 : _a.name) || 'Student',
                ...
                weakestSubject: ((_b = prediction.subjects
                    .filter((x) => x.samples > 0)
                    .sort((a, b) => a.predictedPercent - b.predictedPercent)[0]) === null || _b === void 0 ? void 0 : _b.subject) || null,
            };
        });
        ...
        const tests = yield mongo_1.SSLCMockTest.find({
...
function seedMasterPrepPlans() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const count = yield mongo_1.SSLCPrepPlan.countDocuments();
            if (count > 0)
                return;
            const masterPlans = [
                {
            ...
            yield mongo_1.SSLCPrepPlan.insertMany(masterPlans);
        }
        catch (err) {
